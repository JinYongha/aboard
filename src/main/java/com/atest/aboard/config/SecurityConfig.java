...
//                        .requestMatchers("/login", "/register", "/resources/**", "/member/register").permitAll() // 회원가입 페이지 허용
                        .requestMatchers("/", "/login", "/register", "/resources/**", "/member/register").permitAll() // 루트 URL과 로그인, 회원가입은 접근 가능
...
//                            System.out.println("Login Successful: Redirecting to /board/aboard");
//                            response.sendRedirect("/board/aboard"); // 로그인 후 리다이렉트
//                        })
//                        .permitAll()
//                )
